"use strict";var w=Object.defineProperty;var F=Object.getOwnPropertyDescriptor;var M=Object.getOwnPropertyNames;var V=Object.prototype.hasOwnProperty;var j=(n,t)=>{for(var e in t)w(n,e,{get:t[e],enumerable:!0})},H=(n,t,e,s)=>{if(t&&typeof t=="object"||typeof t=="function")for(let i of M(t))!V.call(n,i)&&i!==e&&w(n,i,{get:()=>t[i],enumerable:!(s=F(t,i))||s.enumerable});return n};var q=n=>H(w({},"__esModule",{value:!0}),n);var Z={};j(Z,{default:()=>x});module.exports=q(Z);var E=require("obsidian");var m=class{baseUrl;token;constructor(t,e){this.baseUrl=t,this.token=e||null}setToken(t){this.token=t}async request(t,e,s,i=!1){let a={};this.token&&(a.Authorization=`Bearer ${this.token}`),s&&!i&&(a["Content-Type"]="application/json");let r=0,c=3;for(;r<c;)try{let l=await fetch(`${this.baseUrl}${e}`,{method:t,headers:a,body:i?s:s?JSON.stringify(s):void 0});if(!l.ok){let o=await l.json().catch(()=>({message:"Request failed"}));throw l.status>=400&&l.status<500?new g(o.code||"ERROR",o.message,l.status):new g(o.code||"SERVER_ERROR",o.message||"Server error",l.status)}return l.json()}catch(l){if(l instanceof g&&l.status>=400&&l.status<500||(r++,r>=c))throw l;await new Promise(o=>setTimeout(o,2**r*1e3))}throw new Error("Max retries exceeded")}async getDeviceCode(){return this.request("POST","/auth/device-code")}async pollDeviceCode(t){return this.request("GET",`/auth/poll/${t}`)}async revokeToken(){return this.request("POST","/auth/revoke")}async getSites(){return this.request("GET","/sites")}async createSite(t){return this.request("POST","/sites",{subdomain:t})}async getSite(t){return this.request("GET",`/sites/${t}`)}async getNotes(t){return this.request("GET",`/sites/${t}/notes`)}async fullSync(t,e){return this.request("POST",`/sites/${t}/sync`,{notes:e})}async getImages(t){return this.request("GET",`/sites/${t}/images`)}async uploadImage(t,e,s){let i=new FormData,a=new Blob([e],{type:this.getMimeType(s)});return i.append("image",a,s),this.request("POST",`/sites/${t}/images`,i,!0)}async publish(t,e=!1){let s=e?"?force=true":"";return this.request("POST",`/sites/${t}/publish${s}`)}getMimeType(t){let e=t.split(".").pop()?.toLowerCase();return{png:"image/png",jpg:"image/jpeg",jpeg:"image/jpeg",gif:"image/gif",webp:"image/webp",svg:"image/svg+xml"}[e||""]||"application/octet-stream"}},g=class extends Error{code;status;constructor(t,e,s){super(e),this.code=t,this.status=s,this.name="ApiError"}};var R=require("obsidian"),b=class extends R.Modal{code;verificationUrl;api;pollInterval=null;onSuccess;onCancel;constructor(t,e,s,i,a,r){super(t),this.code=e,this.verificationUrl=s,this.api=i,this.onSuccess=a,this.onCancel=r}onOpen(){let{contentEl:t}=this;t.empty(),t.addClass("publish-device-code-modal"),t.createEl("h2",{text:"Connect Your Vault"}),t.createDiv({cls:"publish-instructions"}).createEl("p",{text:"Enter this code in your browser to connect:"}),t.createDiv({cls:"publish-code-display"}).createEl("span",{text:this.code,cls:"publish-code"});let i=t.createDiv({cls:"publish-button-container"}),a=i.createEl("button",{text:"Copy Code",cls:"mod-cta"});a.addEventListener("click",()=>{navigator.clipboard.writeText(this.code),a.textContent="Copied!",setTimeout(()=>{a.textContent="Copy Code"},2e3)}),i.createEl("button",{text:"Open in Browser"}).addEventListener("click",()=>{window.open(this.verificationUrl,"_blank")});let c=t.createDiv({cls:"publish-status"});c.createEl("p",{text:"Waiting for authorization..."}),t.createEl("button",{text:"Cancel",cls:"publish-cancel-button"}).addEventListener("click",()=>{this.close(),this.onCancel()}),this.startPolling(c),this.addStyles()}startPolling(t){this.pollInterval=setInterval(async()=>{try{let e=await this.api.pollDeviceCode(this.code);e.status==="completed"&&e.token?(this.stopPolling(),t.empty(),t.createEl("p",{text:"Connected successfully!",cls:"publish-status-success"}),setTimeout(()=>{this.close(),this.onSuccess({token:e.token,email:e.email||"",siteId:e.siteId,subdomain:e.subdomain})},1e3)):e.status==="expired"&&(this.stopPolling(),t.empty(),t.createEl("p",{text:"Code expired. Please try again.",cls:"publish-status-error"}))}catch{}},2e3)}stopPolling(){this.pollInterval&&(clearInterval(this.pollInterval),this.pollInterval=null)}onClose(){this.stopPolling();let{contentEl:t}=this;t.empty()}addStyles(){let t=document.createElement("style");t.textContent=`
		...
		`,this.contentEl.appendChild(t)}};var G="https://api.pubit.site",W="https://auth.pubit.site",T=G,Y=W,N=`${Y}/dashboard/`,B="pubit.site",D={};var h=require("obsidian");var y=class extends h.PluginSettingTab{plugin;constructor(t,e){super(t,e),this.plugin=e}display(){let{containerEl:t}=this;t.empty();let e=!!this.plugin.settings.apiToken;if(new h.Setting(t).setName("Connection status").setDesc(e?`Connected as ${this.plugin.settings.userEmail||"Unknown"}`:"Not connected").addButton(s=>{e?s.setButtonText("Disconnect").setWarning().onClick(async()=>{await this.plugin.disconnect(),this.display()}):s.setButtonText("Connect").setCta().onClick(async()=>{await this.plugin.startDeviceCodeFlow(),setTimeout(()=>this.display(),1e3)})}),e&&this.plugin.settings.subdomain){let i=`https://${this.plugin.settings.subdomain}.${B}`;new h.Setting(t).setName("Your site").setDesc(i).addButton(a=>a.setButtonText("Open Site").onClick(()=>{window.open(i,"_blank")})),new h.Setting(t).setName("Dashboard").setDesc("Manage your site, edit menu order, or delete your site").addButton(a=>a.setButtonText("Open Dashboard").onClick(()=>{window.open(N,"_blank")}))}}};function K(n){if(n.replace(/```[\s\S]*?```/g,"").replace(/`[^`]+`/g,"").includes("#publish"))return!0;if(n.startsWith("---")){let e=n.indexOf(`
---`,3);if(e!==-1){let s=n.slice(4,e);if(/^publish:\s*["']?true["']?\s*$/im.test(s))return!0}}return!1}async function L(n){let t=n.vault.getMarkdownFiles(),e=[];for(let s of t){let i=await n.vault.cachedRead(s);K(i)&&e.push(s)}return e}function O(n){return n.path.replace(/\.md$/,"").split("/").map(i=>C(i)).join("/")}function U(n){let t=/!\[\[([^\]|]+)(?:\|[^\]]+)?\]\]/g,e=[],s;for(;(s=t.exec(n))!==null;){let i=s[1].trim();/\.(png|jpg|jpeg|gif|webp|svg)$/i.test(i)&&e.push(i)}return[...new Set(e)]}function _(n,t){let e=n.vault.getFiles(),s=t.toLowerCase(),i=t.split("/").pop()||t,a=i.toLowerCase();for(let r of e)if(r.name===t||r.name===i||r.path===t)return r;for(let r of e)if(r.name.toLowerCase()===s||r.name.toLowerCase()===a)return r;for(let r of e)if(r.path.toLowerCase().endsWith(`/${a}`))return r;if(!/\.(png|jpg|jpeg|gif|webp|svg)$/i.test(t))for(let r of["png","jpg","jpeg","gif","webp","svg"]){let l=`${i}.${r}`.toLowerCase();for(let o of e)if(o.name.toLowerCase()===l)return o}return null}async function X(n,t,e,s,i){i?.("Checking existing images on server...");let a={};try{a=(await t.getImages(e)).images}catch(o){console.error("Failed to fetch server images:",o)}let r=new Set(Object.keys(a)),c=[],l=new Set;for(let o of s){let u=await n.vault.cachedRead(o),p=U(u);for(let d of p)l.add(d)}for(let o of l){if(r.has(o))continue;let u=_(n,o);u&&c.push({filename:o,file:u})}for(let o=0;o<c.length;o++){let{filename:u,file:p}=c[o];i?.(`Uploading image ${o+1}/${c.length}: ${u}`);try{let d=await n.vault.readBinary(p);await t.uploadImage(e,d,u)}catch(d){console.error(`[Pubit] Failed to upload image ${u}:`,d)}}}async function $(n,t,e,s){try{s?.("Collecting notes...",5);let i=await L(n);if(i.length===0)return s?.("No notes to publish",100),{success:!0,uploaded:0,deleted:0,errors:[]};s?.("Syncing images...",10),await X(n,t,e,i,l=>s?.(l,15)),s?.("Preparing notes...",20);let a=[];for(let l of i){let o=O(l),u=await n.vault.cachedRead(l);a.push({slug:o,content:u})}s?.(`Uploading ${a.length} notes...`,30);let r=await t.fullSync(e,a);s?.("Publishing...",80);let c=await t.publish(e);return{success:r.errors.length===0,uploaded:r.uploaded,deleted:r.deleted,errors:r.errors.map(l=>`${l.slug}: ${l.message}`),siteUrl:c.url}}catch(i){return{success:!1,uploaded:0,deleted:0,errors:[i instanceof Error?i.message:"Unknown error"]}}}var x=class extends E.Plugin{settings=D;api=null;statusBarItem=null;settingsTab=null;ribbonIcon=null;hasPendingChanges=!1;styleEl=null;async onload(){await this.loadSettings(),this.api=new m(T,this.settings.apiToken),this.registerStyles(),this.ribbonIcon=this.addRibbonIcon("upload-cloud","Publish notes",async()=>{await this.publish()}),this.registerEvent(this.app.vault.on("modify",()=>this.markPendingChanges())),this.registerEvent(this.app.vault.on("create",()=>this.markPendingChanges())),this.registerEvent(this.app.vault.on("delete",()=>this.markPendingChanges())),this.registerEvent(this.app.vault.on("rename",()=>this.markPendingChanges())),this.addCommand({id:"publish-sync",name:"Sync and publish",callback:async()=>{await this.publish()}}),this.addCommand({id:"publish-connect",name:"Connect vault",callback:async()=>{await this.startDeviceCodeFlow()}}),this.addCommand({id:"publish-disconnect",name:"Disconnect vault",callback:async()=>{await this.disconnect()}}),this.settingsTab=new y(this.app,this),this.addSettingTab(this.settingsTab),this.statusBarItem=this.addStatusBarItem(),this.updateStatusBar()}async loadSettings(){this.settings=Object.assign({},D,await this.loadData())}async saveSettings(){await this.saveData(this.settings),this.api&&(this.api=new m(T,this.settings.apiToken)),this.updateStatusBar()}showNotice(t){new E.Notice(t)}updateStatusBar(){this.statusBarItem&&(this.settings.apiToken?this.hasPendingChanges?(this.statusBarItem.setText("Publish: Changes pending"),this.statusBarItem.setAttribute("aria-label","Click ribbon icon to sync")):(this.statusBarItem.setText("Publish: Up to date"),this.statusBarItem.setAttribute("aria-label","All synced")):(this.statusBarItem.setText("Publish: Not connected"),this.statusBarItem.setAttribute("aria-label","Click to connect")))}markPendingChanges(){this.settings.apiToken&&(this.hasPendingChanges||(this.hasPendingChanges=!0,this.updatePendingIndicator(),this.updateStatusBar()))}clearPendingChanges(){this.hasPendingChanges=!1,this.updatePendingIndicator(),this.updateStatusBar()}updatePendingIndicator(){this.ribbonIcon&&(this.hasPendingChanges?(this.ribbonIcon.addClass("has-pending-changes"),this.ribbonIcon.setAttribute("aria-label","Publish notes (changes pending)")):(this.ribbonIcon.removeClass("has-pending-changes"),this.ribbonIcon.setAttribute("aria-label","Publish notes")))}registerStyles(){this.styleEl=document.createElement("style"),this.styleEl.id="publish-plugin-styles",this.styleEl.textContent=`
