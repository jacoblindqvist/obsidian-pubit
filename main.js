"use strict";var P=Object.defineProperty;var K=Object.getOwnPropertyDescriptor;var Q=Object.getOwnPropertyNames;var Z=Object.prototype.hasOwnProperty;var J=(n,t)=>{for(var e in t)P(n,e,{get:t[e],enumerable:!0})},tt=(n,t,e,s)=>{if(t&&typeof t=="object"||typeof t=="function")for(let i of Q(t))!Z.call(n,i)&&i!==e&&P(n,i,{get:()=>t[i],enumerable:!(s=K(t,i))||s.enumerable});return n};var et=n=>tt(P({},"__esModule",{value:!0}),n);var lt={};J(lt,{default:()=>D});module.exports=et(lt);var I=require("obsidian");var v=class{baseUrl;token;constructor(t,e){this.baseUrl=t,this.token=e||null}setToken(t){this.token=t}async request(t,e,s,i=!1){let a={};this.token&&(a.Authorization=`Bearer ${this.token}`),s&&!i&&(a["Content-Type"]="application/json");let r=0,c=5;for(;r<c;)try{let l=await fetch(`${this.baseUrl}${e}`,{method:t,headers:a,body:i?s:s?JSON.stringify(s):void 0});if(!l.ok){let o=await l.json().catch(()=>({message:"Request failed"}));if(l.status===429){let u=l.headers.get("Retry-After"),d=u?Number.parseInt(u,10)*1e3:2**r*1e3;if(r++,r>=c)throw new m("RATE_LIMITED","Rate limited. Try again later.",429);await new Promise(h=>setTimeout(h,Math.min(d,3e4)));continue}throw l.status>=400&&l.status<500?new m(o.code||"ERROR",o.message,l.status):new m(o.code||"SERVER_ERROR",o.message||"Server error",l.status)}return l.json()}catch(l){if(l instanceof m&&l.status>=400&&l.status<500||(r++,r>=c))throw l;await new Promise(o=>setTimeout(o,2**r*1e3))}throw new Error("Max retries exceeded")}async getDeviceCode(){return this.request("POST","/auth/device-code")}async pollDeviceCode(t){return this.request("GET",`/auth/poll/${t}`)}async revokeToken(){return this.request("POST","/auth/revoke")}async getSites(){return this.request("GET","/sites")}async createSite(t){return this.request("POST","/sites",{subdomain:t})}async getSite(t){return this.request("GET",`/sites/${t}`)}async getNotes(t){return this.request("GET",`/sites/${t}/notes`)}async uploadNote(t,e,s,i){let a={"Content-Type":"text/markdown; charset=utf-8","X-Note-Title":encodeURIComponent(s)};this.token&&(a.Authorization=`Bearer ${this.token}`);let r=5;for(let c=0;c<r;c++){let l=await fetch(`${this.baseUrl}/sites/${t}/notes/${e}`,{method:"PUT",headers:a,body:i});if(l.ok)return l.json();if(l.status===429){let u=l.headers.get("Retry-After"),d=u?Number.parseInt(u,10)*1e3:2**c*1e3;await new Promise(h=>setTimeout(h,Math.min(d,3e4)));continue}let o=await l.json().catch(()=>({message:"Upload failed"}));throw new m(o.code||"UPLOAD_FAILED",o.message,l.status)}throw new m("RATE_LIMITED","Rate limited after multiple retries",429)}async deleteNote(t,e){return this.request("DELETE",`/sites/${t}/notes/${e}`)}async getImages(t){return this.request("GET",`/sites/${t}/images`)}async uploadImage(t,e,s){let i=new FormData,a=new Blob([e],{type:this.getMimeType(s)});return i.append("image",a,s),this.request("POST",`/sites/${t}/images`,i,!0)}async publish(t,e=!1){let s=e?"?force=true":"";return this.request("POST",`/sites/${t}/publish${s}`)}getMimeType(t){let e=t.split(".").pop()?.toLowerCase();return{png:"image/png",jpg:"image/jpeg",jpeg:"image/jpeg",gif:"image/gif",webp:"image/webp",svg:"image/svg+xml"}[e||""]||"application/octet-stream"}},m=class extends Error{code;status;constructor(t,e,s){super(e),this.code=t,this.status=s,this.name="ApiError"}};var $=require("obsidian"),w=class extends $.Modal{code;verificationUrl;api;pollInterval=null;onSuccess;onCancel;constructor(t,e,s,i,a,r){super(t),this.code=e,this.verificationUrl=s,this.api=i,this.onSuccess=a,this.onCancel=r}onOpen(){let{contentEl:t}=this;t.empty(),t.addClass("publish-device-code-modal"),t.createEl("h2",{text:"Connect Your Vault"}),t.createDiv({cls:"publish-instructions"}).createEl("p",{text:"Enter this code in your browser to connect:"}),t.createDiv({cls:"publish-code-display"}).createEl("span",{text:this.code,cls:"publish-code"});let i=t.createDiv({cls:"publish-button-container"}),a=i.createEl("button",{text:"Copy Code",cls:"mod-cta"});a.addEventListener("click",()=>{navigator.clipboard.writeText(this.code),a.textContent="Copied!",setTimeout(()=>{a.textContent="Copy Code"},2e3)}),i.createEl("button",{text:"Open in Browser"}).addEventListener("click",()=>{window.open(this.verificationUrl,"_blank")});let c=t.createDiv({cls:"publish-status"});c.createEl("p",{text:"Waiting for authorization..."}),t.createEl("button",{text:"Cancel",cls:"publish-cancel-button"}).addEventListener("click",()=>{this.close(),this.onCancel()}),this.startPolling(c),this.addStyles()}startPolling(t){this.pollInterval=setInterval(async()=>{try{let e=await this.api.pollDeviceCode(this.code);e.status==="completed"&&e.token?(this.stopPolling(),t.empty(),t.createEl("p",{text:"Connected successfully!",cls:"publish-status-success"}),setTimeout(()=>{this.close(),this.onSuccess({token:e.token,email:e.email||"",siteId:e.siteId,subdomain:e.subdomain})},1e3)):e.status==="expired"&&(this.stopPolling(),t.empty(),t.createEl("p",{text:"Code expired. Please try again.",cls:"publish-status-error"}))}catch{}},2e3)}stopPolling(){this.pollInterval&&(clearInterval(this.pollInterval),this.pollInterval=null)}onClose(){this.stopPolling();let{contentEl:t}=this;t.empty()}addStyles(){let t=document.createElement("style");t.textContent=`
		...
		`,this.contentEl.appendChild(t)}};var it="https://api.pubit.site",nt="https://auth.pubit.site",_=it,ot=nt,V=`${ot}/dashboard/`,j="pubit.site",E={publishTag:"publish"};var f=require("obsidian");var T=class extends f.PluginSettingTab{plugin;constructor(t,e){super(t,e),this.plugin=e}display(){let{containerEl:t}=this;t.empty();let e=!!this.plugin.settings.apiToken;if(new f.Setting(t).setName("Connection status").setDesc(e?`Connected as ${this.plugin.settings.userEmail||"Unknown"}`:"Not connected").addButton(s=>{e?s.setButtonText("Disconnect").setWarning().onClick(async()=>{await this.plugin.disconnect(),this.display()}):s.setButtonText("Connect").setCta().onClick(async()=>{await this.plugin.startDeviceCodeFlow(),setTimeout(()=>this.display(),1e3)})}),e&&this.plugin.settings.subdomain){let i=`https://${this.plugin.settings.subdomain}.${j}`;new f.Setting(t).setName("Your site").setDesc(i).addButton(a=>a.setButtonText("Open Site").onClick(()=>{window.open(i,"_blank")})),new f.Setting(t).setName("Dashboard").setDesc("Manage your site, edit menu order, or delete your site").addButton(a=>a.setButtonText("Open Dashboard").onClick(()=>{window.open(V,"_blank")}))}new f.Setting(t).setName("Publish tag").setDesc('Notes with this property set to true in frontmatter will be published (e.g., "publish: true")').addText(s=>s.setPlaceholder("publish").setValue(this.plugin.settings.publishTag||"").onChange(async i=>{this.plugin.settings.publishTag=i||E.publishTag,await this.plugin.saveSettings()}))}};function rt(n,t="publish"){if(!n.startsWith("---"))return!1;let e=n.indexOf(`
---`,3);if(e===-1)return!1;let s=n.slice(4,e),i=t.replace(/[.*+?^${}()|[\]\\]/g,"\\$&");return new RegExp(`^${i}:\\s*["']?true["']?\\s*$`,"im").test(s)}async function H(n,t="publish"){let e=n.vault.getMarkdownFiles(),s=[];for(let i of e){if(i.path.startsWith(".trash/")||i.path.startsWith(".obsidian/"))continue;let a=await n.vault.cachedRead(i);rt(a,t)&&s.push(i)}return s}function q(n){return n.basename}function z(n){return n.path.replace(/\.md$/,"").split("/").map(i=>k(i)).join("/")}function G(n){let t=/!\[\[([^\]|]+)(?:\|[^\]]+)?\]\]/g,e=[],s;for(;(s=t.exec(n))!==null;){let i=s[1].trim();/\.(png|jpg|jpeg|gif|webp|svg)$/i.test(i)&&e.push(i)}return[...new Set(e)]}function W(n,t){let e=n.vault.getFiles(),s=t.toLowerCase(),i=t.split("/").pop()||t,a=i.toLowerCase();for(let r of e)if(r.name===t||r.name===i||r.path===t)return r;for(let r of e)if(r.name.toLowerCase()===s||r.name.toLowerCase()===a)return r;for(let r of e)if(r.path.toLowerCase().endsWith(`/${a}`))return r;if(!/\.(png|jpg|jpeg|gif|webp|svg)$/i.test(t))for(let r of["png","jpg","jpeg","gif","webp","svg"]){let l=`${i}.${r}`.toLowerCase();for(let o of e)if(o.name.toLowerCase()===l)return o}return null}async function at(n,t,e,s,i){i?.("Checking existing images on server...");let a={};try{a=(await t.getImages(e)).images}catch(o){console.error("Failed to fetch server images:",o)}let r=new Set(Object.keys(a)),c=[],l=new Set;for(let o of s){let u=await n.vault.cachedRead(o),d=G(u);for(let h of d)l.add(h)}for(let o of l){if(r.has(o))continue;let u=W(n,o);u&&c.push({filename:o,file:u})}for(let o=0;o<c.length;o++){let{filename:u,file:d}=c[o];i?.(`Uploading image ${o+1}/${c.length}: ${u}`);try{let h=await n.vault.readBinary(d);await t.uploadImage(e,h,u)}catch(h){console.error(`[Pubit] Failed to upload image ${u}:`,h)}}}async function X(n,t,e,s,i="publish"){let a=[];try{s?.("Collecting local notes...",5);let r=await H(n,i);s?.("Fetching remote state...",10);let c=await t.getNotes(e),l=new Map(c.map(p=>[p.slug,p.contentHash]));s?.("Syncing images...",15),await at(n,t,e,r,p=>s?.(p,20)),s?.("Computing changes...",25);let o=[],u=[],d=new Set,h=0;for(let p of r){let g=z(p),b=q(p),y=await n.vault.cachedRead(p),x=await N(y);d.add(g);let L=l.get(g);!L||L!==x?o.push({slug:g,title:b,content:y}):h++}for(let[p]of l)d.has(p)||u.push(p);let R=0;if(o.length>0){let p=o.length;for(let g=0;g<o.length;g++){let b=o[g],y=30+g/p*40;s?.(`Uploading ${g+1}/${p}: ${b.slug.split("/").pop()}...`,y);try{await t.uploadNote(e,b.slug,b.title,b.content),R++}catch(x){a.push(`Upload failed: ${b.slug} - ${x instanceof Error?x.message:"Unknown"}`)}}}let A=0;if(u.length>0){s?.(`Deleting ${u.length} orphaned notes...`,75);for(let p of u)try{await t.deleteNote(e,p),A++}catch(g){a.push(`Delete failed: ${p} - ${g instanceof Error?g.message:"Unknown"}`)}}if(R>0||A>0){s?.("Publishing...",85);let p=await t.publish(e);return{success:a.length===0,uploaded:R,deleted:A,unchanged:h,errors:a,siteUrl:p.url}}s?.("Publishing...",90);let Y=await t.publish(e);return{success:!0,uploaded:0,deleted:0,unchanged:r.length,errors:[],siteUrl:Y.url}}catch(r){return{success:!1,uploaded:0,deleted:0,unchanged:0,errors:[r instanceof Error?r.message:"Unknown error"]}}}var D=class extends I.Plugin{settings=E;api=null;statusBarItem=null;settingsTab=null;ribbonIcon=null;hasPendingChanges=!1;styleEl=null;async onload(){await this.loadSettings(),this.api=new v(_,this.settings.apiToken),this.registerStyles(),this.ribbonIcon=this.addRibbonIcon("upload-cloud","Publish notes",async()=>{await this.publish()}),this.registerEvent(this.app.vault.on("modify",()=>this.markPendingChanges())),this.registerEvent(this.app.vault.on("create",()=>this.markPendingChanges())),this.registerEvent(this.app.vault.on("delete",()=>this.markPendingChanges())),this.registerEvent(this.app.vault.on("rename",()=>this.markPendingChanges())),this.addCommand({id:"publish-sync",name:"Sync and publish",callback:async()=>{await this.publish()}}),this.addCommand({id:"publish-connect",name:"Connect vault",callback:async()=>{await this.startDeviceCodeFlow()}}),this.addCommand({id:"publish-disconnect",name:"Disconnect vault",callback:async()=>{await this.disconnect()}}),this.settingsTab=new T(this.app,this),this.addSettingTab(this.settingsTab),this.statusBarItem=this.addStatusBarItem(),this.updateStatusBar()}async loadSettings(){this.settings=Object.assign({},E,await this.loadData())}async saveSettings(){await this.saveData(this.settings),this.api&&(this.api=new v(_,this.settings.apiToken)),this.updateStatusBar()}showNotice(t){new I.Notice(t)}updateStatusBar(){this.statusBarItem&&(this.settings.apiToken?this.hasPendingChanges?(this.statusBarItem.setText("Publish: Changes pending"),this.statusBarItem.setAttribute("aria-label","Click ribbon icon to sync")):(this.statusBarItem.setText("Publish: Up to date"),this.statusBarItem.setAttribute("aria-label","All synced")):(this.statusBarItem.setText("Publish: Not connected"),this.statusBarItem.setAttribute("aria-label","Click to connect")))}markPendingChanges(){this.settings.apiToken&&(this.hasPendingChanges||(this.hasPendingChanges=!0,this.updatePendingIndicator(),this.updateStatusBar()))}clearPendingChanges(){this.hasPendingChanges=!1,this.updatePendingIndicator(),this.updateStatusBar()}updatePendingIndicator(){this.ribbonIcon&&(this.hasPendingChanges?(this.ribbonIcon.addClass("has-pending-changes"),this.ribbonIcon.setAttribute("aria-label","Publish notes (changes pending)")):(this.ribbonIcon.removeClass("has-pending-changes"),this.ribbonIcon.setAttribute("aria-label","Publish notes")))}registerStyles(){this.styleEl=document.createElement("style"),this.styleEl.id="publish-plugin-styles",this.styleEl.textContent=`
