"use strict";var N=Object.defineProperty;var Z=Object.getOwnPropertyDescriptor;var J=Object.getOwnPropertyNames;var Q=Object.prototype.hasOwnProperty;var tt=(n,t)=>{for(var e in t)N(n,e,{get:t[e],enumerable:!0})},et=(n,t,e,s)=>{if(t&&typeof t=="object"||typeof t=="function")for(let i of J(t))!Q.call(n,i)&&i!==e&&N(n,i,{get:()=>t[i],enumerable:!(s=Z(t,i))||s.enumerable});return n};var st=n=>et(N({},"__esModule",{value:!0}),n);var ut={};tt(ut,{default:()=>T});module.exports=st(ut);var D=require("obsidian");var x=class{baseUrl;token;constructor(t,e){this.baseUrl=t,this.token=e||null}setToken(t){this.token=t}async request(t,e,s,i=!1){let a={};this.token&&(a.Authorization=`Bearer ${this.token}`),s&&!i&&(a["Content-Type"]="application/json");let o=0,c=3;for(;o<c;)try{let u=await fetch(`${this.baseUrl}${e}`,{method:t,headers:a,body:i?s:s?JSON.stringify(s):void 0});if(!u.ok){let r=await u.json().catch(()=>({message:"Request failed"}));throw u.status>=400&&u.status<500?new y(r.code||"ERROR",r.message,u.status):new y(r.code||"SERVER_ERROR",r.message||"Server error",u.status)}return u.json()}catch(u){if(u instanceof y&&u.status>=400&&u.status<500||(o++,o>=c))throw u;await new Promise(r=>setTimeout(r,2**o*1e3))}throw new Error("Max retries exceeded")}async getDeviceCode(){return this.request("POST","/auth/device-code")}async pollDeviceCode(t){return this.request("GET",`/auth/poll/${t}`)}async revokeToken(){return this.request("POST","/auth/revoke")}async getSites(){return this.request("GET","/sites")}async createSite(t){return this.request("POST","/sites",{subdomain:t})}async getSite(t){return this.request("GET",`/sites/${t}`)}async getNotes(t){return this.request("GET",`/sites/${t}/notes`)}async syncNotes(t,e){return this.request("POST",`/sites/${t}/sync`,e)}async getImages(t){return this.request("GET",`/sites/${t}/images`)}async uploadImage(t,e,s){let i=new FormData,a=new Blob([e],{type:this.getMimeType(s)});return i.append("image",a,s),this.request("POST",`/sites/${t}/images`,i,!0)}async publish(t,e=!1){let s=e?"?force=true":"";return this.request("POST",`/sites/${t}/publish${s}`)}getMimeType(t){let e=t.split(".").pop()?.toLowerCase();return{png:"image/png",jpg:"image/jpeg",jpeg:"image/jpeg",gif:"image/gif",webp:"image/webp",svg:"image/svg+xml"}[e||""]||"application/octet-stream"}},y=class extends Error{code;status;constructor(t,e,s){super(e),this.code=t,this.status=s,this.name="ApiError"}};var F=require("obsidian"),w=class extends F.Modal{code;verificationUrl;api;pollInterval=null;onSuccess;onCancel;constructor(t,e,s,i,a,o){super(t),this.code=e,this.verificationUrl=s,this.api=i,this.onSuccess=a,this.onCancel=o}onOpen(){let{contentEl:t}=this;t.empty(),t.addClass("publish-device-code-modal"),t.createEl("h2",{text:"Connect Your Vault"}),t.createDiv({cls:"publish-instructions"}).createEl("p",{text:"Enter this code in your browser to connect:"}),t.createDiv({cls:"publish-code-display"}).createEl("span",{text:this.code,cls:"publish-code"});let i=t.createDiv({cls:"publish-button-container"}),a=i.createEl("button",{text:"Copy Code",cls:"mod-cta"});a.addEventListener("click",()=>{navigator.clipboard.writeText(this.code),a.textContent="Copied!",setTimeout(()=>{a.textContent="Copy Code"},2e3)}),i.createEl("button",{text:"Open in Browser"}).addEventListener("click",()=>{window.open(this.verificationUrl,"_blank")});let c=t.createDiv({cls:"publish-status"});c.createEl("p",{text:"Waiting for authorization..."}),t.createEl("button",{text:"Cancel",cls:"publish-cancel-button"}).addEventListener("click",()=>{this.close(),this.onCancel()}),this.startPolling(c),this.addStyles()}startPolling(t){this.pollInterval=setInterval(async()=>{try{let e=await this.api.pollDeviceCode(this.code);e.status==="completed"&&e.token?(this.stopPolling(),t.empty(),t.createEl("p",{text:"Connected successfully!",cls:"publish-status-success"}),setTimeout(()=>{this.close(),this.onSuccess({token:e.token,email:e.email||"",siteId:e.siteId,subdomain:e.subdomain})},1e3)):e.status==="expired"&&(this.stopPolling(),t.empty(),t.createEl("p",{text:"Code expired. Please try again.",cls:"publish-status-error"}))}catch{}},2e3)}stopPolling(){this.pollInterval&&(clearInterval(this.pollInterval),this.pollInterval=null)}onClose(){this.stopPolling();let{contentEl:t}=this;t.empty()}addStyles(){let t=document.createElement("style");t.textContent=`
		...
		`,this.contentEl.appendChild(t)}};var nt="https://api.pubit.site",ot="https://auth.pubit.site",L=nt,rt=ot,H=`${rt}/dashboard/`,z="pubit.site",_={};var b=require("obsidian");var R=class extends b.PluginSettingTab{plugin;constructor(t,e){super(t,e),this.plugin=e}display(){let{containerEl:t}=this;t.empty(),t.createEl("h2",{text:"Publish Settings"});let e=!!this.plugin.settings.apiToken;if(new b.Setting(t).setName("Connection Status").setDesc(e?`Connected as ${this.plugin.settings.userEmail||"Unknown"}`:"Not connected").addButton(s=>{e?s.setButtonText("Disconnect").setWarning().onClick(async()=>{await this.plugin.disconnect(),this.display()}):s.setButtonText("Connect").setCta().onClick(async()=>{await this.plugin.startDeviceCodeFlow(),setTimeout(()=>this.display(),1e3)})}),e&&this.plugin.settings.subdomain){let i=`https://${this.plugin.settings.subdomain}.${z}`;new b.Setting(t).setName("Your Site").setDesc(i).addButton(a=>a.setButtonText("Open Site").onClick(()=>{window.open(i,"_blank")})),new b.Setting(t).setName("Dashboard").setDesc("Manage your site, edit menu order, or delete your site").addButton(a=>a.setButtonText("Open Dashboard").onClick(()=>{window.open(H,"_blank")}))}}};function at(n){if(n.replace(/```[\s\S]*?```/g,"").replace(/`[^`]+`/g,"").includes("#publish"))return!0;if(n.startsWith("---")){let e=n.indexOf(`
---`,3);if(e!==-1){let s=n.slice(4,e);if(/^publish:\s*["']?true["']?\s*$/im.test(s))return!0}}return!1}async function M(n){let t=n.vault.getMarkdownFiles(),e=[];for(let s of t){let i=await n.vault.cachedRead(s);at(i)&&e.push(s)}return e}function G(n){return n.basename}function W(n){return n.path.replace(/\.md$/,"").split("/").map(i=>B(i)).join("/")}function Y(n){let t=/!\[\[([^\]|]+)(?:\|[^\]]+)?\]\]/g,e=[],s;for(;(s=t.exec(n))!==null;){let i=s[1].trim();/\.(png|jpg|jpeg|gif|webp|svg)$/i.test(i)&&e.push(i)}return[...new Set(e)]}function K(n,t){let e=n.vault.getFiles(),s=t.toLowerCase(),i=t.split("/").pop()||t,a=i.toLowerCase();for(let o of e)if(o.name===t||o.name===i||o.path===t)return o;for(let o of e)if(o.name.toLowerCase()===s||o.name.toLowerCase()===a)return o;for(let o of e)if(o.path.toLowerCase().endsWith(`/${a}`))return o;if(!/\.(png|jpg|jpeg|gif|webp|svg)$/i.test(t))for(let o of["png","jpg","jpeg","gif","webp","svg"]){let u=`${i}.${o}`.toLowerCase();for(let r of e)if(r.name.toLowerCase()===u)return r}return null}async function lt(n,t){let e={create:[],update:[],delete:[]},s=await M(n),i=new Map;for(let o of t)i.set(o.slug,o);let a=new Set;for(let o of s){let c=G(o),u=W(o),r=await n.vault.cachedRead(o),p=await U(r);a.add(u);let d=i.get(u);d?d.contentHash!==p&&e.update.push({file:o,slug:u,title:c,content:r}):e.create.push({file:o,slug:u,title:c,content:r})}for(let o of t)a.has(o.slug)||e.delete.push({slug:o.slug});return e}async function ct(n,t,e,s,i){i?.("Checking existing images on server...");let a={};try{let r=await t.getImages(e);a=r.images,i?.(`Found ${r.count} images on server`)}catch(r){console.error("Failed to fetch server images:",r)}let o=new Map(Object.entries(a)),c=[],u=new Set;for(let r of s){let p=await n.vault.cachedRead(r),d=Y(p);for(let h of d)u.add(h)}for(let r of u){if(o.has(r))continue;let p=K(n,r);p&&c.push({filename:r,file:p})}c.length>0&&i?.(`Uploading ${c.length} new images...`);for(let r=0;r<c.length;r++){let{filename:p,file:d}=c[r];i?.(`Uploading image ${r+1}/${c.length}: ${p}`);try{let h=await n.vault.readBinary(d),E=await t.uploadImage(e,h,p);o.set(p,E.url)}catch(h){console.error(`[Pubit] Failed to upload image ${p}:`,h),i?.(`Failed to upload ${p}`)}}return o}async function X(n,t,e,s){let i=[];try{s?.("Fetching remote notes...");let a=await t.getNotes(e);s?.("Computing changes...");let o=await M(n),c=await lt(n,a),u=await ct(n,t,e,o,s);if(c.create.length+c.update.length+c.delete.length===0){s?.("No note changes to sync"),s?.("Publishing...");let l=await t.publish(e);return{success:!0,created:0,updated:0,deleted:0,errors:[],siteUrl:l.url}}let p=50,d=0,h=0,E=0,P=c.create.map(l=>({slug:l.slug,title:l.title,content:l.content})),I=c.update.map(l=>({slug:l.slug,title:l.title,content:l.content})),k=c.delete.map(l=>({slug:l.slug}));for(let l=0;l<P.length;l+=p){let f=P.slice(l,l+p);s?.(`Creating notes ${l+1}-${Math.min(l+p,P.length)}...`);let v={create:f,update:[],delete:[]},m=await t.syncNotes(e,v);d+=m.created.length;for(let g of m.errors)i.push(`Create error: ${g.title||g.slug} - ${g.message}`)}for(let l=0;l<I.length;l+=p){let f=I.slice(l,l+p);s?.(`Updating notes ${l+1}-${Math.min(l+p,I.length)}...`);let v={create:[],update:f,delete:[]},m=await t.syncNotes(e,v);h+=m.updated.length;for(let g of m.errors)i.push(`Update error: ${g.title||g.slug} - ${g.message}`)}for(let l=0;l<k.length;l+=p){let f=k.slice(l,l+p);s?.(`Deleting notes ${l+1}-${Math.min(l+p,k.length)}...`);let v={create:[],update:[],delete:f},m=await t.syncNotes(e,v);E+=m.deleted.length;for(let g of m.errors)i.push(`Delete error: ${g.slug} - ${g.message}`)}s?.("Publishing...");let A=await t.publish(e);return A.status==="error"&&i.push(`Publish error: ${A.error?.message||"Unknown error"}`),{success:i.length===0,created:d,updated:h,deleted:E,errors:i,siteUrl:A.url}}catch(a){return{success:!1,created:0,updated:0,deleted:0,errors:[a instanceof Error?a.message:"Unknown error"]}}}var T=class extends D.Plugin{settings=_;api=null;statusBarItem=null;settingsTab=null;ribbonIcon=null;hasPendingChanges=!1;async onload(){await this.loadSettings(),this.api=new x(L,this.settings.apiToken),this.registerStyles(),this.ribbonIcon=this.addRibbonIcon("upload-cloud","Publish notes",async()=>{await this.publish()}),this.registerEvent(this.app.vault.on("modify",()=>this.markPendingChanges())),this.registerEvent(this.app.vault.on("create",()=>this.markPendingChanges())),this.registerEvent(this.app.vault.on("delete",()=>this.markPendingChanges())),this.registerEvent(this.app.vault.on("rename",()=>this.markPendingChanges())),this.addCommand({id:"publish-sync",name:"Sync and publish",callback:async()=>{await this.publish()}}),this.addCommand({id:"publish-connect",name:"Connect vault",callback:async()=>{await this.startDeviceCodeFlow()}}),this.addCommand({id:"publish-disconnect",name:"Disconnect vault",callback:async()=>{await this.disconnect()}}),this.settingsTab=new R(this.app,this),this.addSettingTab(this.settingsTab),this.statusBarItem=this.addStatusBarItem(),this.updateStatusBar()}async loadSettings(){this.settings=Object.assign({},_,await this.loadData())}async saveSettings(){await this.saveData(this.settings),this.api&&(this.api=new x(L,this.settings.apiToken)),this.updateStatusBar()}showNotice(t){new D.Notice(t)}updateStatusBar(){this.statusBarItem&&(this.settings.apiToken?this.hasPendingChanges?(this.statusBarItem.setText("Publish: Changes pending"),this.statusBarItem.setAttribute("aria-label","Click ribbon icon to sync")):(this.statusBarItem.setText("Publish: Up to date"),this.statusBarItem.setAttribute("aria-label","All synced")):(this.statusBarItem.setText("Publish: Not connected"),this.statusBarItem.setAttribute("aria-label","Click to connect")))}markPendingChanges(){this.settings.apiToken&&(this.hasPendingChanges||(this.hasPendingChanges=!0,this.updatePendingIndicator(),this.updateStatusBar()))}clearPendingChanges(){this.hasPendingChanges=!1,this.updatePendingIndicator(),this.updateStatusBar()}updatePendingIndicator(){this.ribbonIcon&&(this.hasPendingChanges?(this.ribbonIcon.addClass("has-pending-changes"),this.ribbonIcon.setAttribute("aria-label","Publish notes (changes pending)")):(this.ribbonIcon.removeClass("has-pending-changes"),this.ribbonIcon.setAttribute("aria-label","Publish notes")))}registerStyles(){let t=document.createElement("style");t.id="publish-plugin-styles",t.textContent=`
		...
		`,document.head.appendChild(t)}async startDeviceCodeFlow(){if(!this.api){this.showNotice("Plugin not initialized");return}try{let t=await this.api.getDeviceCode();new w(this.app,t.code,t.verificationUrl,this.api,async s=>{this.settings.apiToken=s.token,this.settings.userEmail=s.email,this.settings.siteId=s.siteId,this.settings.subdomain=s.subdomain,await this.saveSettings(),this.api?.setToken(s.token),this.settingsTab?.display();let i=s.subdomain?` Your site: ${s.subdomain}.pubit.site`:"";this.showNotice(`Successfully connected!${i}`)},()=>{}).open()}catch(t){this.showNotice(`Failed to start connection: ${t instanceof Error?t.message:"Unknown error"}`)}}async disconnect(){if(!this.api||!this.settings.apiToken){this.showNotice("Not connected");return}let t=this.settings.subdomain;try{await this.api.revokeToken()}catch{}this.settings.apiToken=void 0,this.settings.siteId=void 0,this.settings.userEmail=void 0,this.settings.subdomain=void 0,await this.saveSettings(),t?this.showNotice(`Disconnected. Your site remains online at ${t}.pubit.site. To delete it, visit the dashboard.`):this.showNotice("Disconnected")}async publish(){if(!this.api){this.showNotice("Plugin not initialized");return}if(!this.settings.apiToken){this.showNotice("Please connect your vault first"),await this.startDeviceCodeFlow();return}if(!this.settings.siteId){this.showNotice("No site configured. Please disconnect and reconnect.");return}let t=new S(this.app);t.open();try{let e=await X(this.app,this.api,this.settings.siteId,s=>{t.setStatus(s)});e.success?(t.showSuccess(e.siteUrl||"",{created:e.created,updated:e.updated,deleted:e.deleted}),this.clearPendingChanges()):t.showError(e.errors.join(`
