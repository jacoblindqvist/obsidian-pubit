"use strict";var P=Object.defineProperty;var K=Object.getOwnPropertyDescriptor;var Q=Object.getOwnPropertyNames;var Z=Object.prototype.hasOwnProperty;var J=(n,t)=>{for(var e in t)P(n,e,{get:t[e],enumerable:!0})},tt=(n,t,e,s)=>{if(t&&typeof t=="object"||typeof t=="function")for(let i of Q(t))!Z.call(n,i)&&i!==e&&P(n,i,{get:()=>t[i],enumerable:!(s=K(t,i))||s.enumerable});return n};var et=n=>tt(P({},"__esModule",{value:!0}),n);var lt={};J(lt,{default:()=>D});module.exports=et(lt);var I=require("obsidian");var v=class{baseUrl;token;constructor(t,e){this.baseUrl=t,this.token=e||null}setToken(t){this.token=t}async request(t,e,s,i=!1){let r={};this.token&&(r.Authorization=`Bearer ${this.token}`),s&&!i&&(r["Content-Type"]="application/json");let a=0,c=5;for(;a<c;)try{let l=await fetch(`${this.baseUrl}${e}`,{method:t,headers:r,body:i?s:s?JSON.stringify(s):void 0});if(!l.ok){let o=await l.json().catch(()=>({message:"Request failed"}));if(l.status===429){let u=l.headers.get("Retry-After"),d=u?Number.parseInt(u,10)*1e3:2**a*1e3;if(a++,a>=c)throw new m("RATE_LIMITED","Rate limited. Try again later.",429);await new Promise(h=>setTimeout(h,Math.min(d,3e4)));continue}throw l.status>=400&&l.status<500?new m(o.code||"ERROR",o.message,l.status):new m(o.code||"SERVER_ERROR",o.message||"Server error",l.status)}return l.json()}catch(l){if(l instanceof m&&l.status>=400&&l.status<500||(a++,a>=c))throw l;await new Promise(o=>setTimeout(o,2**a*1e3))}throw new Error("Max retries exceeded")}async getDeviceCode(){return this.request("POST","/auth/device-code")}async pollDeviceCode(t){return this.request("GET",`/auth/poll/${t}`)}async revokeToken(){return this.request("POST","/auth/revoke")}async getSites(){return this.request("GET","/sites")}async createSite(t){return this.request("POST","/sites",{subdomain:t})}async getSite(t){return this.request("GET",`/sites/${t}`)}async getNotes(t){return this.request("GET",`/sites/${t}/notes`)}async uploadNote(t,e,s,i){let r={"Content-Type":"text/markdown; charset=utf-8","X-Note-Title":encodeURIComponent(s)};this.token&&(r.Authorization=`Bearer ${this.token}`);let a=5;for(let c=0;c<a;c++){let l=await fetch(`${this.baseUrl}/sites/${t}/notes/${e}`,{method:"PUT",headers:r,body:i});if(l.ok)return l.json();if(l.status===429){let u=l.headers.get("Retry-After"),d=u?Number.parseInt(u,10)*1e3:2**c*1e3;await new Promise(h=>setTimeout(h,Math.min(d,3e4)));continue}let o=await l.json().catch(()=>({message:"Upload failed"}));throw new m(o.code||"UPLOAD_FAILED",o.message,l.status)}throw new m("RATE_LIMITED","Rate limited after multiple retries",429)}async deleteNote(t,e){return this.request("DELETE",`/sites/${t}/notes/${e}`)}async getImages(t){return this.request("GET",`/sites/${t}/images`)}async uploadImage(t,e,s){let i=new FormData,r=new Blob([e],{type:this.getMimeType(s)});return i.append("image",r,s),this.request("POST",`/sites/${t}/images`,i,!0)}async publish(t,e=!1){let s=e?"?force=true":"";return this.request("POST",`/sites/${t}/publish${s}`)}getMimeType(t){let e=t.split(".").pop()?.toLowerCase();return{png:"image/png",jpg:"image/jpeg",jpeg:"image/jpeg",gif:"image/gif",webp:"image/webp",svg:"image/svg+xml"}[e||""]||"application/octet-stream"}},m=class extends Error{code;status;constructor(t,e,s){super(e),this.code=t,this.status=s,this.name="ApiError"}};var B=require("obsidian"),w=class extends B.Modal{code;verificationUrl;api;pollInterval=null;onSuccess;onCancel;constructor(t,e,s,i,r,a){super(t),this.code=e,this.verificationUrl=s,this.api=i,this.onSuccess=r,this.onCancel=a}onOpen(){let{contentEl:t}=this;t.empty(),t.addClass("publish-device-code-modal"),t.createEl("h2",{text:"Connect Your Vault"}),t.createDiv({cls:"publish-instructions"}).createEl("p",{text:"Enter this code in your browser to connect:"}),t.createDiv({cls:"publish-code-display"}).createEl("span",{text:this.code,cls:"publish-code"});let i=t.createDiv({cls:"publish-button-container"}),r=i.createEl("button",{text:"Copy Code",cls:"mod-cta"});r.addEventListener("click",()=>{navigator.clipboard.writeText(this.code),r.textContent="Copied!",setTimeout(()=>{r.textContent="Copy Code"},2e3)}),i.createEl("button",{text:"Open in Browser"}).addEventListener("click",()=>{window.open(this.verificationUrl,"_blank")});let c=t.createDiv({cls:"publish-status"});c.createEl("p",{text:"Waiting for authorization..."}),t.createEl("button",{text:"Cancel",cls:"publish-cancel-button"}).addEventListener("click",()=>{this.close(),this.onCancel()}),this.startPolling(c),this.addStyles()}startPolling(t){this.pollInterval=setInterval(async()=>{try{let e=await this.api.pollDeviceCode(this.code);e.status==="completed"&&e.token?(this.stopPolling(),t.empty(),t.createEl("p",{text:"Connected successfully!",cls:"publish-status-success"}),setTimeout(()=>{this.close(),this.onSuccess({token:e.token,email:e.email||"",siteId:e.siteId,subdomain:e.subdomain})},1e3)):e.status==="expired"&&(this.stopPolling(),t.empty(),t.createEl("p",{text:"Code expired. Please try again.",cls:"publish-status-error"}))}catch{}},2e3)}stopPolling(){this.pollInterval&&(clearInterval(this.pollInterval),this.pollInterval=null)}onClose(){this.stopPolling();let{contentEl:t}=this;t.empty()}addStyles(){let t=document.createElement("style");t.textContent=`
